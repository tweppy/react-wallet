...
function Home() {
    ...
    if (card !== activeCard) {
      return (
        <Card 
          cardData={card}
          key={card.cardNumber}
          handleActiveCard={() => handleActiveCard(card)}
        />
      );
    }
  });
  ...
  return (
    <div className='container'>
      <Top 
        title='E-WALLET'
        text={ activeCard !== vendorData[0] ? 'ACTIVE CARD' : 'NO ACTIVE CARD' } 
      />
      ...
      <button 
        ...
        onClick={ handleNav }>ADD A NEW CARD</button>
    </div>
  );
}
